...
public class InzeratQueryServiceImpl extends AbstractQueryService implements InzeratQueryService {

    public InzeratQueryServiceImpl(HttpClient httpClient, String queryUrl) {
        super(httpClient, queryUrl);
    }

    @Override
    public boolean shouldScrapeInzeratFor(String inzeratUrl) {
        return shouldScrapeDataFor(inzeratUrl);
    }

    @Override
    protected String makeShouldScrapeDataUrl(String inzeratIdentifier) {
        return RetApiConfig.makeShouldScrapeInzeratURL(inzeratIdentifier);
    }
}
