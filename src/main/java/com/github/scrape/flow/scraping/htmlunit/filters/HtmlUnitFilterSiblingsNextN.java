...
public class HtmlUnitFilterSiblingsNextN implements Filter<DomNode> {

    private final int n;

    public HtmlUnitFilterSiblingsNextN(int n) {
        if (n <= 0) {
    ...
    public List<DomNode> filter(List<DomNode> allNextSiblings) {
        return new HtmlUnitFilterFirstNth(n).filter(allNextSiblings);
    }
