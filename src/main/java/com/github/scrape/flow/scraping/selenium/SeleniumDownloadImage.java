...
@Log4j2
public class SeleniumDownloadImage extends SeleniumScrapingStep<SeleniumDownloadImage>
        implements CollectingParsedValueToModelStep<SeleniumDownloadImage, BufferedImage>,
        MakingHttpRequests {

    SeleniumDownloadImage() {
    }

    @Override
    protected SeleniumDownloadImage copy() {
        return copyFieldValuesTo(new SeleniumDownloadImage());
    }
    ...
    @Override
    public <T> SeleniumDownloadImage collectValue(BiConsumer<T, BufferedImage> modelMutation, Class<T> containerType) {
        return addCollector(new Collector(modelMutation, BufferedImage.class, containerType, AccumulatorType.ONE));
    }

    @Override
    public <T> SeleniumDownloadImage collectValues(BiConsumer<T, BufferedImage> modelMutation, Class<T> containerType) {
