...
@Log4j2
public class SeleniumSendKeys extends SeleniumScrapingStep<SeleniumSendKeys>
        implements LoadingNewPage {

    private final String text;

    SeleniumSendKeys(String text) {
        this.text = text;
    }

    @Override
    protected SeleniumSendKeys copy() {
        return copyFieldValuesTo(new SeleniumSendKeys(text));
    }
    ...
    protected StepOrder execute(ScrapingContext ctx, ScrapingServices services) {
        ...
        Runnable runnable = () -> {
            ctx.getWebElement().sendKeys(text);
