...
interface HtmlUnitStepCollectingParsedValuesToModel<C> {

    org.apache.logging.log4j.Logger log = org.apache.logging.log4j.LogManager.getLogger(HtmlUnitStepCollectingParsedValuesToModel.class);

    /**
     * Sets up the operation that will set the actual scraped data to model object
     */
    <T> C collect(BiConsumer<T, String> modelMutation, Class<T> containerType);


    default void setParsedStringToModel(CollectorSetups collectorSetups, ParsingContext ctx, String parsedValue, String stepName) {
        try {
            List<CollectorSetup> stringCollectors = collectorSetups.getAccumulators().stream()
                    .filter(co -> co.getModelClass().equals(String.class))
